...
final class DocumentParser<R, Seq extends List<R>, M extends Map<R, R?>> {
  DocumentParser(
    this._scanner, {
  ...
  /// Alias builder
  final AliasFunction<R, R> aliasFunction;

  /// Sequence builder
  final ListFunction<R, Seq> listFunction;
  ...
  void _trackAnchor<T extends ParserDelegate<R>>(
    T delegate,
  ...
  AliasDelegate<R, R> _referenceAlias(
    Alias property, {
  ...
  T? _nullOrAlias<T extends ParserDelegate<R>>(
    ParsedProperty property, {
    required int indentLevel,
    required int indent,
    required RuneOffset start,
  ...
  ///
  /// See [parseNext].
  (PreScalar scalar, ScalarDelegate<R> delegate) _parseScalar(
    ScalarEvent event, {
  ...
  ///
  /// See [_parseBlockMap] and [_parseFlowMap].
  AliasDelegate<R, R>? _aliasKeyOrNull(
    ParsedProperty property, {
    required int indentLevel,
    required int indent,
  ...
  T _parseFlowNode<T extends ParserDelegate<R>>({
  ...
  (ParserDelegate<R>? key, ParserDelegate<R>? value) _parseFlowMapEntry(
    ...
    if (inferNextEvent(
      ...
      if (property is Alias) {
        ...
        value = ignoreValue(_scanner.charAtCursor)
            ? (nullScalarDelegate(
                indentLevel: valueLevel,
                indent: minIndent,
  ...
  void _parseFlowMap(
    ...
    while (_scanner.canChunkMore) {
      ...
      if (!delegate.accept((key.parsed(), value?.parsed()))) {
        _onMapDuplicate(
          key.start,
  ...
  bool _checkTrailingEntry(
    SequenceDelegate<R, Seq> sequence, {
  ...
  ParserDelegate<R> _flowCompactToMap(
    ParserDelegate<R> key,
    ParserDelegate<R>? value,
  ) {
    final map =
    ...
    return map as ParserDelegate<R>;
  }
  ...
  void _parseFlowSequence(
    SequenceDelegate<R, Seq> delegate, {
    required bool forceInline,
  }) {
    ...
    while (_scanner.canChunkMore) {
      ...
      switch (event) {
        ...
        case FlowCollectionEvent.startExplicitKey:
          {
            ...
            delegate.accept(_flowCompactToMap(key, value).parsed());
          }
        ...
        default:
          {
            ParserDelegate<R>? keyOrElement = _aliasKeyOrNull(
              property,
              indentLevel: indentLevel,
              indent: indent,
            ...
            if (keyOrElement.encounteredLineBreak ||
                inferNextEvent(
                    ...
                    FlowCollectionEvent.startEntryValue) {
              _trackAnchor(keyOrElement, property);
              delegate.accept(keyOrElement.parsed());
              break;
            }
            ...
            delegate.accept(_flowCompactToMap(keyOrElement, value).parsed());
          }
      }
  ...
  ///
  ///
  _BlockNode<R> _parseBlockScalarWildcard(
    ScalarEvent event, {
    ...
    if (!_scanner.canChunkMore ||
        ...
        hasLineBreak ||
        indentDidChange) {
      _trackAnchor(delegate, scalarProperty);
  ...
  _BlockNode<R> _parseWildcardBlockMap(
    ParserDelegate<R> blockKey, {
    ...
    if (eventIfScalar == ScalarEvent.startFlowDoubleQuoted ||
      ...
      if (!_scanner.canChunkMore ||
        ...
        return (
          delegate: blockKey,
          nodeInfo: (exitIndent: greedyIndent, docMarker: DocumentMarker.none),
        );
      }
    }
    ...
    if (keyProperty.isMultiline) {
      _trackAnchor(map as ParserDelegate<R>, keyProperty);
    }

    return (delegate: map as ParserDelegate<R>, nodeInfo: mapInfo);
  }

  /// Parses a flow collection embedded within a block collection.
  _BlockNode<R> _parseEmbeddedFlowCollection(
    ...
    _trackAnchor(flowNode, flowProperty);
      ...
      case BlockCollectionEvent.startEntryValue:
        {
          ...
          ParserDelegate<R>? nonExistentKey = nullScalarDelegate(
            indentLevel: indentLevel,
            indent: fixedInlineIndent,
            startOffset: startOffset,
            resolver: scalarFunction,
          ...
          _trackAnchor(map as ParserDelegate<R>, mapProperty);

          node = map as ParserDelegate<R>;
        }
      ...
      case BlockCollectionEvent.startBlockListEntry when !forceInlined:
        {
          ...
          _trackAnchor(list as ParserDelegate<R>, blockNodeProperty);
          node = list as ParserDelegate<R>;
        }
      ...
      case BlockCollectionEvent.startExplicitKey when !forceInlined:
        {
          ...
          _trackAnchor(map as ParserDelegate<R>, blockNodeProperty);
          node = map as ParserDelegate<R>;
        }
  ...
  (bool shouldExit, _BlockNodeInfo info, ParserDelegate<R> key)
  _parseExplicitBlockKey({required int indentLevel, required int mapIndent}) {
    ...
    if (shouldExit && !parsedProperty.isAlias) {
      ...
      final delegate = nullScalarDelegate(
        indentLevel: indentLevel,
        indent: mapIndent,
        startOffset: keyOffset,
        resolver: scalarFunction,
      )..updateEndOffset = preKeyHasIndent ? lineInfo.start : lineInfo.current;

      _trackAnchor(delegate, parsedProperty);
  ...
  /// using the `?` character.
  _BlockEntry<R> _parseExplicitBlockEntry({
    required int indentLevel,
    required int indent,
  }) {
  ...
  _ImplicitBlockValue<R> _parseImplicitBlockValue({
  ...
  _BlockEntry<R> _parseImplicitBlockEntry(
    ParserDelegate<R>? key, {
    required int parentIndent,
    required int parentIndentLevel,
    required ParsedProperty keyProperty,
    ...
    if (key == null && event != BlockCollectionEvent.startEntryValue) {
      ...
      if (delegate case ScalarDelegate(
        scalarStyle: ScalarStyle.plain,
        :final String content, // TODO: Iffy on this. Add test for this
  ...
  _BlockNodeInfo _parseBlockMap(
    MappingDelegate<R, M> map,
    ParserDelegate<R>? firstImplicitKey,
  ...
  _BlockNode<R> _parseBlockSequenceEntry({
    required int indentLevel,
    required int laxIndent,
    required int fixedInlineIndent,
    required bool forceInlined,
  ...
  _BlockNodeInfo _parseBlockSequence(SequenceDelegate<R, Seq> sequence) {
    ...
    do {
      ...
      sequence.accept(delegate.parsed());
  ...
  (bool didParse, T? parsed) parseNext<T>() {
    ...
    if (!_scanner.canChunkMore) return (false, null);
    ...
    final parsed = root.parsed();

    if (parsed is YamlSourceNode) {
      return (
        true,
        YamlDocument._(
              _currentIndex,
